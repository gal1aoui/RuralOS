import { ACTIVITIES, ADDONS, COMPLIANCE, CONTEXT_POINTS, HOST_OFFERS, LODGING, PACKAGES, SEASONS, SEASON_NAMES, VISA } from "./data";
import { LOCALES, eur, tx, type Lang, type T } from "./i18n";
import { KIND_LABELS, LAND_STATS, LEASE, PARCELS, PROJECTS } from "./land";
...
function build(lang: Lang): KnowledgeEntry[] {
  ...
  k.push({ id: "land-lease", title: t({ en: "Leasing land for an agricultural project (5 to 25 years)", es: "Arrendar tierra para un proyecto agrario (de 5 a 25 años)" }), link: "/land", text: t({ en: `We lease villagers' parcels to chestnut, organic, cattle, beekeeping and forest projects for ${LEASE.minYears} to 25 years (never days or months: ${LEASE.minYears} years is the legal minimum under Ley 49/2003, renewed by 5-year periods). The owner keeps ${LEASE.ownerShare * 100}% of the rent, paid yearly; RuralRiver keeps ${LEASE.commission * 100}%. The project pays a one-off set-up fee of ${e(LEASE.setupFee)} (parcel search, owner consolidation, lease drafting, Banco de Terras and Concello paperwork, first local hires) and ${e(LEASE.managementYear)} a year for management. Rents are around Galicia's average of ${e(LEASE.galiciaAvgRent)}/ha/year. Every lease has a local-hiring clause. Use the map and lease builder at /land.`, es: `Arrendamos fincas de vecinos a proyectos de castaña, ecológico, ganadería, apicultura y bosque de ${LEASE.minYears} a 25 años (nunca días ni meses: ${LEASE.minYears} años es el mínimo legal según la Ley 49/2003, prorrogable por periodos de 5). El propietario se queda el ${LEASE.ownerShare * 100} % del canon, cobrado cada año; RuralRiver el ${LEASE.commission * 100} %. El proyecto paga una puesta en marcha de ${e(LEASE.setupFee)} (búsqueda de parcelas, agrupación de propietarios, contrato, trámites con el Banco de Terras y el Concello, primeras contrataciones) y ${e(LEASE.managementYear)} al año de gestión. Los cánones rondan la media gallega de ${e(LEASE.galiciaAvgRent)}/ha/año. Cada contrato lleva cláusula de contratación local. Usa el mapa y el configurador en /land.` }) });
  for (const p of PROJECTS) {
    k.push({ id: `proj-${p.id}`, title: `${t({ en: "Agricultural project", es: "Proyecto agrario" })}: ${t(p.label)}`, link: "/land", text: `${t(p.blurb)} ${t({ en: `From ${p.minHa} ha; roughly ${p.jobsPerHa} permanent and ${p.seasonalPerHa} seasonal local jobs per hectare (estimate).`, es: `Desde ${p.minHa} ha; unos ${p.jobsPerHa} empleos fijos y ${p.seasonalPerHa} de temporada por hectárea (estimación).` })}` });
  }
  k.push({ id: "parcels", title: t({ en: "Example parcels in San Xoán de Río", es: "Fincas de ejemplo en San Xoán de Río" }), link: "/land", text: `${t({ en: "Illustrative parcels (real places, example plots; owners confirm availability)", es: "Fincas ilustrativas (lugares reales, parcelas de ejemplo; los propietarios confirman disponibilidad)" })}: ${PARCELS.map((p) => `${t(p.name)} (${p.place}, ${p.ha} ha, ${t(KIND_LABELS[p.kind])}, ${p.altitude} m, ${t({ en: "unused since", es: "sin uso desde" })} ${p.unusedSince}, ≈${e(p.rentPerHa)}/ha/${t({ en: "year", es: "año" })})`).join("; ")}.` });
  k.push({ id: "land-facts", title: t({ en: "Land and farming in Galicia: the numbers", es: "Tierra y agro en Galicia: las cifras" }), link: "/land", text: LAND_STATS.map((s) => `${tx(s.value, lang)} ${t(s.label)}`).join(". ") + "." });
